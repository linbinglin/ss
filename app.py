...
2. **画面具象度**：是否有不寻常具体细节
3. **台词活人感**：是否像真人说的话
4. **台词个性化**：遮住名字能否辨别谁说的；是否所有人听起来一样
5. **视觉翻译完成度**：是否有台词替代画面叙事
6. **分镜密度**：每10-14秒是否≥5个动作事件
7. **因果链完整度**：伏笔和转折是否有因果
8. **情绪过山车强度**：情绪起伏是否到位
9. **上下镜衔接流畅度**：分镜之间是否连贯
10. **时长准确度**：标注时长与实算时长偏差是否≤±2秒
11. **台词嵌入度**：台词是否嵌入画面流的精确时间点（而非单独分行）
12. **台词情绪描写**：每句台词前是否有说话者的情绪/表情/身体状态描写

【重点检查项：台词三合一】
对每句台词检查：
- 嵌入位置：这句话在动作流的哪个时间点说出？读者能否判断？
- 说话状态：说这句话时的表情、情绪、身体动作是否描写了？
- 角色DNA：这句话符合角色的说话习惯吗？
...
with mt[1]:
    if bt["质量检查"]:
        if en not in st.session_state.episodes: st.warning(f"⚠️ 第{en}集未生成")
        else:
