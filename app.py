...
逐镜检查10个维度（1-10分），特别注意：

【台词个性化专项检查】
请逐个角色检查：
- 该角色在原著中的说话风格是什么？
- 剧本中该角色的台词是否还原了这种风格？
- 是否所有角色的台词听起来都差不多？（如果是→严重问题）
- 是否有角色的台词被过度精简导致失去个性？

输出每条分镜检查报告+五个敌对视角+整集汇总。
7分以下必须给出具体修改方案。"""

# ══════════════════════════════════════════════════════════════
# 台词优化专用prompt (完全重写 - 核心修复)
# ══════════════════════════════════════════════════════════════
...
请从中提取每个角色的说话DNA作为优化依据。
"""

    return f"""你是一个台词优化专家。你的任务是优化第{ep}集剧本中的台词。

═══════════════════════════════════════
【核心原则：台词优化≠台词精简！】
═══════════════════════════════════════

台词优化的本质是——让每个角色说出更像"他/她自己"的话。

不是缩短，不是精简，不是文学化，而是【个性化】+【潜台词化】+【情绪匹配】。
...
第一步：角色台词DNA确认
对本集每个出场角色，先确认其说话DNA：
- 句式习惯：长句？短句？碎片化？文绉绉？粗糙直接？
- 口头禅/高频词：有没有角色标志性的用词？
- 语气特征：温柔？暴躁？怯懦？傲慢？嬉皮笑脸？
- 情绪变化规律：这个角色紧张时说话会怎样？生气时呢？心软时呢？

第二步：逐句台词检查
对每一句台词执行以下检查：

检查1——个性标签："遮住角色名，能否通过这句话猜出是谁？"
- 能→保留
- 不能→重写，加入该角色的说话DNA特征

检查2——情绪匹配："这个角色在这个情绪下会怎么说话？"
- 暴躁角色生气时→可能更短促、更脏话、更有攻击性
- 话痨角色紧张时→可能更碎片化、更语无伦次、重复更多
- 沉默角色被触动时→可能只说半句就咽回去
- 每个角色有不同的情绪表达方式，不能统一缩短！

检查3——潜台词深度："这句话表面说的是什么？真实意图是什么？有没有缝隙？"
- "别信他们" → 潜台词可以是"其实我在意你"
- "小丧尸" → 潜台词可以是"我担心你但嘴硬"
- "嫌弃你" → 潜台词可以是"护短又得意"
- 让潜台词更丰富，不是删减台词

检查4——关系动态："这个角色面对这个对象时说话方式对吗？"
- 面对喜欢的人可能嘴硬+身体泄露真心
- 面对权威可能压抑+措辞谨慎
- 面对仇人可能反而异常平静

第三步：输出优化版
- 对每句修改的台词，标注【✏️优化】并简要说明优化理由
- 说明理由时必须关联角色驱动卡（"因为XX的说话DNA是……"）
...
请输出优化后的完整剧本。保持分镜结构不变，只优化台词和内心OS部分。
每处修改标注原因。最后附一个【台词优化总结】，说明每个角色的台词做了什么方向的调整。"""


def build_visual_optimization_prompt(ep, script):
    return f"""只优化第{ep}集画面描写。

要求：
1. 每个分镜必须有一个"不寻常的具体细节"（钥匙弹跳声、指甲划过桌面、灯光闪烁频率）
2. 用声音锚定空间（沉默场景更需要一个微小声音来放大沉默）
3. 光源必须具体（不是"灯光"，是"手机屏幕的冷白光照亮了她的下巴"）
4. 身体失控比表情形容词有力（不是"她很震惊"，是"杯子从指缝滑下去"）
5. 反差动作比直球动作有力（不是"生气摔门"，是"极其控制地一根根手指松开"）
6. 每个分镜≥3个连续动作事件（有时间流动感，不是静态描写）
7. 台词部分保持不变，只调整画面/动作/环境描写

当前剧本：
{script}

输出优化后完整剧本，画面修改处标注【🎨优化】。"""


def build_emotion_optimization_prompt(ep, script):
    return f"""只优化第{ep}集的情绪节奏。

要求：
1. 开场15秒是否有足够的情绪冲击？如果不够→增强
2. 集内是否有至少一次情绪急转？如果没有→制造
3. 结尾悬念钩子是否让人"不看下集不行"？如果不够→加强
4. 检查情绪曲线：不能一直高或一直低，必须有起伏
5. 根据场景类型选用合适手法：
   - 爽点场景→弹簧法（先压到最低再反杀）
   - 心动场景→磁铁法（空间入侵+不受控生理反应+被打断）
   - 虐心场景→错位法（信息差让观众知道更多+极平静的动作）
   - 紧张场景→橡皮筋法（异象→虚惊→真爆发）
6. ≥65%情绪转折来自人物互动/环境碰撞，不是靠旁白或独白
7. 台词和画面结构基本保持，主要调整情绪节奏和铺排

当前剧本：
{script}

输出优化后完整剧本，情绪调整处标注【❤️优化】。"""
# ============================================================
# 侧边栏
# ============================================================
with st.sidebar:
